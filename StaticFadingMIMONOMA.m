classdef StaticFadingMIMONOMA < handle
    ...
    methods
        function obj = FastFading(...
                ...
                ~,...                                              % Maximum Doppler shift: Velocity_kmh/3.6*CarrierFrequency/2.998e8
            ...
            obj.PHY.MaximumDopplerShift             = 0;  % Fixed for static fading (no mobility)
        ...
        function NewRealization(obj)
            % Static Rayleigh fading realization (no Doppler/mobility)
            numDelays = length(obj.Implementation.PowerDelayProfileNormalized);
            obj.ImpulseResponse = zeros(1, numDelays, obj.Nr.rxAntennas, obj.Nr.txAntennas);
        
            for iTx = 1:obj.Nr.txAntennas
                for iRx = 1:obj.Nr.rxAntennas
                    obj.ImpulseResponse(1,:,iRx,iTx) = ...
                        sqrt(obj.Implementation.PowerDelayProfileNormalized) .* ...
                        (1/sqrt(2)) .* (randn(1, numDelays) + 1j*randn(1, numDelays));
                end
            end
        end
        ...
        function convolvedSignal = Convolution(obj, signal)
            % Time-invariant convolution for static fading
            N = size(signal,1);
            convolvedSignal = zeros(N + size(obj.ImpulseResponse,2)-1, obj.Nr.rxAntennas);
        
            for iTx = 1:obj.Nr.txAntennas
                for iRx = 1:obj.Nr.rxAntennas
                    convolvedSignal(:,iRx) = convolvedSignal(:,iRx) + ...
                        conv(signal(:,iTx), squeeze(obj.ImpulseResponse(1,:,iRx,iTx)));
                end
            end
            convolvedSignal = convolvedSignal(1:N,:);  % Trim tail
        end
