classdef MIMONOMA_PilotCancellation < handle
   ...
   properties (SetAccess = private)
       ...
       NrTransmittedSymbols = 0;
       PilotToDataPowerOffset = 1;
       AuxiliaryToDataPowerOffset = 0;
       DataPowerReduction = 1;
       SIR_dB = [];
   ...
   methods
      ...
      function obj = MIMONOMA_PilotCancellation(varargin)
          ...
          switch Method
              case 'Auxiliary'                  
                  ...
                  %if nargin > 5
                  %    nUsers = varargin{6};
                  %    % This would require stacking PilotMatrix and FBMCMatrix per user
                  %end

                   % Assume PilotMatrix is now cell array of size {nUsers,1}
                  if iscell(PilotMatrix)
                      FBMCMatrixUser = cell(nUsers,1);
                      PrecodingMatrixUser = cell(nUsers,1);
                      for u = 1:nUsers
                          % Apply original logic per user
                          % Recursively create PrecodingMatrixUser{u} and FBMCMatrixUser{u}
                      end
                      % Combine later
                  end
          ...
          obj.NrUsers = nUsers; 
          obj.NrActivePilots = NrPilotSymbols;
